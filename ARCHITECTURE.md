...
- *Deterministic (`checks.ts`):* `fetch(url, {method: "HEAD"})` with timeout per listed project URL → `{url, status, live: boolean, latencyMs}`. For GitHub repos: unauthenticated GitHub REST (`/repos/{owner}/{repo}`, `/contents`, `/commits?per_page=30`) → file tree, README presence, language breakdown, plus 2–3 representative source files (capped bytes). Commit recency is computed here in plain code, not judged by the LLM: a `RepoActivity` block (days since last commit from committer dates, commit counts over 90/365-day windows, activity span, the `checkedAtIso` the ages were computed against) derived from the fetched commit sample — window counts are honest lower bounds when the sample hits the page cap. Its numeric subset (no timestamps) is forwarded to the blinded Assessor with the verifier findings, and the reviewer sees the facts in the run view's "Verification facts" table.
